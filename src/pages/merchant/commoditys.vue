...
export default {
  ...
  methods: {
    ...
    renderAction(h, params) {
      const {
        status = '', isBanner, isGuessLike, isWeekHot
      } = params.row;
      ...
      return h(
        ...
        [
          ...
          h('Button', {
            props: {
              type: isWeekHot ? 'warning' : 'primary',
              size: 'small'
            },
            style: {
              marginRight: '8px',
            ...
            on: {
              click: () => {
                this.toggleStatus(params.row, 'weekHot', isWeekHot);
              }
            }
          }, '本周热推'),
          h('Button', {
            props: {
              type: isGuessLike ? 'warning' : 'primary',
              size: 'small'
            },
            style: {
              marginBottom: '8px'
            ...
            on: {
              click: () => {
                this.toggleStatus(params.row, 'guessLike', isGuessLike);
              }
            }
          }, '猜你喜欢')
        ],
    ...
    viewDetail(id) {
      const query = {
        come: this.routerName
      };
      if (id) {
        query.id = id;
      }
      this.$router.push({ name: 'commoditysEditor', query });
    },
    editor({ id }) {
      this.viewDetail(id);
    },
