    ...
    <Form :model="formDatas"
      ...
      <div class="m-form-contain">
        ...
        <Row>
          <Col span="11">
            <FormItem label="封面图" required>
              <OImgUpload :desc="imgDesc" :maxNum="1" ref="coverImgUpload"
        ...
        <Row>
          <Col span="11">
            <div class="sort-item">
              <FormItem label="原价" required>
                <InputNumber v-model="formDatas.originalAmount" number></InputNumber>
              </FormItem>
            </div>
          </Col>
        </Row>
        <Row>
          <Col span="11">
            <div class="sort-item">
              <FormItem label="现价" required>
                <InputNumber v-model="formDatas.currentAmount" number></InputNumber>
              </FormItem>
            </div>
          </Col>
        </Row>
...
import { Select, Option, InputNumber } from 'iview';
import { commoditysCons } from '@/constant';
...
export default {
  ...
  components: {
    OImgUpload, Select, Option, QuillEditor, InputNumber
  },
  mounted() {
    this.init();
  },
  ...
  methods: {
    init() {
      ...
      this.$nextTick(() => {
        this.initBcs();
        this.querySellers();
        this.imgDesc = commoditysCons[this.routerFromName].imgDesc;
        this.formDatas = { ...this.formDatas };
      });
    },
    initBcs() {
      const bc = [];
      bc.push({ title: commoditysCons[this.routerFromName].title });
      if (this.isEditor) {
        this.getData();
        bc.push({ title: '编辑' });
      } else {
    ...
    querySellers(keywords) {
      ...
      this.$http().post('/seller/list', {
        keywords,
        productType: commoditysCons[this.routerFromName].type
      }).then((data) => {
    ...
    submit() {
      ...
      if (!isPass) return;
      this.$http()
        .post('/product/save', { ...params, type: commoditysCons[this.routerFromName].type }, { isJson: true })
        .then(() => {
    ...
    isVali(params) {
      ...
      let msg = '';
      const {
        name, coverFileId, description, sellerId, originalAmount, currentAmount
      } = params;
      if (!description) {
        msg = '请填写简介！';
      }
