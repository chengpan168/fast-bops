...
export default {
  ...
  methods: {
    ...
    toggle(row) {
      const { id, status } = row;
      this.$http()
        .post('/wc/updateStatus', { id, status: statusReverseMap[status] })
        .then(() => {
    ...
    viewDetail(id) {
      const query = id ? { id } : {};
      this.$router.push({ name: 'editorWeddingCases', query });
    },
    editor({ id }) {
      this.viewDetail(id);
    },
    showDel(row) {
      ...
      this.delAction = () => {
        this.$http().post('/wc/remove', { id: row.id }).then(() => {
